    ...
    <div class="rounded-3xl border border-slate-100 bg-white p-6 shadow-soft sm:p-8">
        ...
        <form class="mt-6 grid gap-3 md:grid-cols-4" method="GET" action="{{ route('admin.turnitin.index') }}">
            ...
            <div class="flex gap-2">
                ...
                <a class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-700 shadow-soft transition hover:bg-slate-50" href="{{ route('admin.turnitin.index') }}">Reset</a>
            </div>
        </form>
            ...
            <table class="min-w-full text-left text-sm">
                ...
                @foreach($submissions as $item)
                    <tr>
                        ...
                        <td class="py-4 pr-4">
                            <form method="POST" action="{{ route('admin.turnitin.update', $item) }}" enctype="multipart/form-data" class="grid gap-2">
                                @csrf
                                @method('PUT')
                                ...
                                <input name="similarity_percent" type="number" min="0" max="100" value="{{ old('similarity_percent', $item->similarity_percent) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-soft outline-none ring-emerald-200 transition focus:border-emerald-300 focus:ring-4" placeholder="Similarity %">
                                <input name="report_pdf" type="file" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-soft outline-none ring-emerald-200 transition focus:border-emerald-300 focus:ring-4">
